...
----------------------------------------------------------------------------
WANG, Bingheng, 3 Sept. 2020, at Control & Simulation Lab, ECE Dept. NUS
...
def Train():
    ...
    if not os.path.exists("trained_data_1st_order"):
        os.makedirs("trained_data_1st_order")
    ...
    PATH0   = "trained_data_1st_order/initial_model_"+str(key2)+".pt"
    ...
    while delta_cost >= eps: #flag ==0
        ...
        for key in train_set:
            ...
            for j in range(n_start,n_start+100): # these data points covers a limited range of velocity
                ...
                dldw, loss_track = uavNMHE.ChainRule(ground_truth, xmhe_traj, X_opt, weight_grad) # dldw: gradient of the loss w.r.t the parameterized MHE tuning parameters
                ...
                if n_ep> 0: # start learning at the second episode. The first episode is set to be unlearned for comparison
                    ...
                    print('Trained=', n_ep, 'sample=', it, 'number of nn paras=', params, 'percentage=',params/25e3)
        ...
        if n_ep>0 and mean_loss>mean_loss0+5: # 15 only for trial 6 otherwise 5
            break
        
        mean_loss0 = mean_loss
        print('learned', n_ep, 'mean_loss=', mean_loss)
        if n_ep >=120: # stop learning as it is too time-consuming
            break
        n_ep += 1
        if not os.path.exists("trained_data_1st_order"):
            os.makedirs("trained_data_1st_order")
        np.save('trained_data_1st_order/Mean_loss_'+str(key2), Mean_Loss)
        PATH1 = "trained_data_1st_order/Trained_model_"+str(key2)+".pt"
        torch.save(model_QR, PATH1)
        np.save('trained_data_1st_order/Trained episodes_'+str(key2), Trained_episode)

    # Loss function
    if n_ep>1:
        trainingtime = (TM.time() - start_time0)
        np.save('trained_data_1st_order/trainingtime_1st_neuromhe_'+str(key2),trainingtime)
        print("trainingtime=--- %s s ---" % format(trainingtime,'.2f'))
    # Mean_loss = np.load('trained_data_1st_order/Mean_loss.npy')
    ...
    plt.grid()
    plt.savefig('trained_data_1st_order/mean_loss_train_1st_order_'+str(key2)+'.png',dpi=600)
...
def Evaluate(key):
    ...
    PATH1 = "trained_data_1st_order/Trained_model_1.pt"
    ...
    for j in range(n_start,N_ev):
        ...
        Bemnn_Fb   = np.vstack((Bemnn_Fx,Bemnn_Fy,Bemnn_Fz))
        ...
        print('sample=', j, 'Dis_x=', format(df_t[0, 0],'.5f'), 'df_Bmh_x=', format(df_Bmh[0, 0],'.5f'), 'Dis_y=', format(df_t[1, 0],'.5f'), 'df_Imh_y=', format(df_Bmh[1, 0],'.5f'), 'Dis_z=', format(df_t[2, 0],'.5f'), 'df_Imh_z=', format(df_Bmh[2, 0],'.5f'))
        ...
        print('sample=', j,'E_BEM_fx=',format(E_BEM_fx[j-n_start],'.5f'),'res_fx=',format(res_fx,'.5f'),'error_x=',format(E_BEM_fx[j-n_start]-res_fx,'.5f'))
        print('sample=', j,'E_BEM_fy=',format(E_BEM_fy[j-n_start],'.5f'),'res_fy=',format(res_fy,'.5f'),'error_y=',format(E_BEM_fy[j-n_start]-res_fy,'.5f'))
        print('sample=', j,'E_BEM_fz=',format(E_BEM_fz[j-n_start],'.5f'),'res_fz=',format(res_fz,'.5f'),'error_z=',format(E_BEM_fz[j-n_start]-res_fz,'.5f'))
        # Sum the loss
        loss_track = np.reshape(loss_track, (1))
        sum_loss += loss_track
    ...
    np.save('trained_data_1st_order/Time_'+str(key), Time)
    np.save('trained_data_1st_order/Tunable_para_'+str(key), Tunable_para)
    np.save('trained_data_1st_order/Gamma1_'+str(key), gamma1)
    np.save('trained_data_1st_order/Gamma2_'+str(key), gamma2)
    np.save('trained_data_1st_order/Gt_fx_'+str(key), Gt_fx)
    np.save('trained_data_1st_order/Gt_fy_'+str(key), Gt_fy)
    np.save('trained_data_1st_order/Gt_fz_'+str(key), Gt_fz)
    np.save('trained_data_1st_order/Gt_fxy_'+str(key), Gt_fxy)
    np.save('trained_data_1st_order/Gt_f_'+str(key), Gt_f)
    np.save('trained_data_1st_order/Bemnn_fx_'+str(key), Bemnn_fx)
    np.save('trained_data_1st_order/Bemnn_fy_'+str(key), Bemnn_fy)
    np.save('trained_data_1st_order/Bemnn_fxy_'+str(key), Bemnn_fxy)
    np.save('trained_data_1st_order/Bemnn_fz_'+str(key), Bemnn_fz)
    np.save('trained_data_1st_order/Bemnn_f_'+str(key), Bemnn_f)
    np.save('trained_data_1st_order/Bemnn_tx_'+str(key), Bemnn_tx)
    np.save('trained_data_1st_order/Bemnn_ty_'+str(key), Bemnn_ty)
    np.save('trained_data_1st_order/Bemnn_txy_'+str(key), Bemnn_txy)
    np.save('trained_data_1st_order/Bemnn_tz_'+str(key), Bemnn_tz)
    np.save('trained_data_1st_order/Bemnn_t_'+str(key), Bemnn_t)
    np.save('trained_data_1st_order/Gt_tx_'+str(key), Gt_tx)
    np.save('trained_data_1st_order/Gt_ty_'+str(key), Gt_ty)
    np.save('trained_data_1st_order/Gt_txy_'+str(key), Gt_txy)
    np.save('trained_data_1st_order/Gt_t_'+str(key), Gt_t)
    np.save('trained_data_1st_order/Gt_tz_'+str(key), Gt_tz)
    np.save('trained_data_1st_order/fx_mhe_'+str(key), fx_mhe)
    np.save('trained_data_1st_order/fy_mhe_'+str(key), fy_mhe)
    np.save('trained_data_1st_order/fxy_mhe_'+str(key), fxy_mhe)
    np.save('trained_data_1st_order/f_mhe_'+str(key), f_mhe)
    np.save('trained_data_1st_order/fz_mhe_'+str(key), fz_mhe)
    np.save('trained_data_1st_order/tx_mhe_'+str(key), tx_mhe)
    np.save('trained_data_1st_order/ty_mhe_'+str(key), ty_mhe)
    np.save('trained_data_1st_order/txy_mhe_'+str(key),txy_mhe)
    np.save('trained_data_1st_order/t_mhe_'+str(key), t_mhe)
    np.save('trained_data_1st_order/tz_mhe_'+str(key), tz_mhe)
    np.save('trained_data_1st_order/error_fxy_mhe_v_'+str(key), E_MHE_fxy)
    np.save('trained_data_1st_order/error_f_mhe_v_'+str(key), E_MHE_f)
    np.save('trained_data_1st_order/error_txy_mhe_v_'+str(key), E_MHE_txy)
    np.save('trained_data_1st_order/error_t_mhe_v_'+str(key), E_MHE_t)
    np.save('trained_data_1st_order/error_fxy_bem_v_'+str(key), E_BEM_fxy)
    np.save('trained_data_1st_order/error_f_bem_v_'+str(key), E_BEM_f)
    np.save('trained_data_1st_order/error_txy_bem_v_'+str(key), E_BEM_txy)
    np.save('trained_data_1st_order/error_t_bem_v_'+str(key), E_BEM_t)
...
key2 = '10'
meanloss_TR = np.load('trained_data/Mean_loss_TR_NeuroMHE_'+str(key2)+'.npy')
meanloss_TR_0 = meanloss_TR[0]
print('meanloss_TR_untrained_'+str(key2)+'=',meanloss_TR_0)
